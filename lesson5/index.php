...
<div class="lesson5">
	...
	echo '<div id="lesson5_gallery">';
	while ($row = mysqli_fetch_array($result)) {
		echo '<div class="img_item">';
		echo '<img id="'. $row['id'] .'" src=" ' . THUMB_PATH . $row['thumb_name'] . ' " alt="миниатюра">';
		echo '<p>';
		echo '<span class="views" onclick="alert(\'Это количество просмотров\')"></span>';
		echo '<i>' . $row['views'] . '</i>';
		echo '<span class="likes" id="like_' . $row['id'] . '" onclick="like_plus(' . $row['id'] . ')"></span>';
		echo '<i>' . $row['likes'] . '</i>';
		echo '</p>';
		echo '</div>';
		echo '<div class="lesson5_wrapper"><div id="lesson5_full_size_show"></div></div>';
	}
	echo '</div>';
